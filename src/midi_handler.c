...
void process_midi_message(const snd_seq_event_t *event, 
    ...
    if (current_part->BASS.sequence.count > 0 && current_part->BASS.listen_channel == channel) {
    ...
    if (current_part->blooper_cc_count > 0 && current_part->blooper_listen_channel == channel) {
        play_BLOOPER_cc(midi, song_set, state);
    }
...
void send_midi_cc(midi_interface_t *midi, uint8_t channel, uint8_t cc_number, uint8_t value, const char *cc_name, int value_index, int total_values) {
    ...
    // Configurer l'événement CONTROL CHANGE
    snd_seq_ev_set_source(&ev, midi->output_port);
    snd_seq_ev_set_subs(&ev);
    snd_seq_ev_set_direct(&ev);
    ...
    ev.data.control.param = cc_number;
    ev.data.control.value = value;

    // Envoyer CC
    int result = snd_seq_event_output(midi->seq_handle, &ev);
    if (result < 0) {
        printf("[ERROR] Failed to send CC: %s\n", snd_strerror(result));
        return;
    }
